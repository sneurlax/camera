...
use camera_control::{
    camera_control_close, camera_control_enumerate, camera_control_frame_free, camera_control_open,
    camera_control_start_stream, camera_control_stop_stream, camera_control_string_free,
};
...
extern "C" fn on_frame(
    ...
    camera_control_frame_free(data, length);
}
...
fn main() {
    ...
    let handle = camera_control_open(c_id.as_ptr());
    if handle <= 0 {
        eprintln!("open failed: {handle}");
        return;
    }

    let rc = camera_control_start_stream(handle, on_frame);
    if rc != 0 {
    ...
    camera_control_stop_stream(handle);
    camera_control_close(handle);
    ...
    if let Some((bytes, w, h)) = LAST_FRAME.lock().unwrap().take() {
        ...
        write_ppm("/tmp/camera_control_frame.ppm", &bytes, w, h);
    } else if count == 0 {
