...
//
// Pure-Dart CLI usage of camera_control over the FFI backend. Build the native
// library first (from the repo root):
//
//   cargo build --release --manifest-path native/camera_control_native/Cargo.toml
//
// then run:
//
//   dart run packages/camera_control/example/camera_control_example.dart
//
// It prints the device list, opens the first camera, and saves its first frame
// to /tmp/camera_control_frame.ppm (a portable format any image viewer reads).
...
Future<void> main(List<String> args) async {
  ...
  try {
    ...
    const path = '/tmp/camera_control_frame.ppm';
